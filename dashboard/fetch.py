...
def search_by(s, dim, since, until, limit=25000):
    ...
    ПОТОЛОК БЫЛ НАШ, А СПИСАН НА GOOGLE — и это главное, что здесь надо
    знать. Здесь стояло `limit=250` при том, что посуточный ряд тянется с
    лимитом 25 000: то есть предел ставили не платформа и не квота, а мы.
    Строки Google отдаёт по убыванию кликов, а ВНУТРИ нулевого блока — по
    алфавиту. Значит обрезались не «редкие запросы вообще», а всё, что
    начинается с букв после начала алфавита: у сайта GS Pay Tables из 230
    нулевых строк не осталось НИ ОДНОЙ на «gs», «opm», «salary»,
    «locality», «pay», «table», «how», «what» — весь его собственный
    словарь. 96,4% показов оставались без названного запроса.

    И потеря была объяснена анонимизацией — здесь, в шапке отдаваемого CSV
    и на самой доске, то есть следующему читателю было сказано не искать.
    Анонимизация существует и правда срезает часть запросов, но она НЕ
    сортирует по алфавиту и не объясняет пропажу целого словаря.

    Оставшийся разрыв между суммой этого разреза и итогом посуточного ряда
    — вот он действительно анонимизация, и доля увиденного печатается
    рядом, чтобы её было видно, а не подразумевалось.
    """
    rows = []
    for key, prop in sorted(GSC.items()):
        url = ("https://searchconsole.googleapis.com/webmasters/v3/sites/"
...
def main():
    ...
    for _name, _dim, _col in (("search_queries.csv", "query", "query"),
        ...
        _write(_name, ["site", _col, "impressions", "clicks", "ctr", "position"],
               _rows,
               "Разрез Search Console по %s, окно %s. Показов в разрезе %d "
               "из %d в итоге (%.0f%%). Разрыв — анонимизация Google: редкие "
               "запросы он не отдаёт вовсе. До 18.09.2026 здесь стояло, что "
               "это «не потеря данных, а её предел», и это было неправдой: "
               "разрез обрезал НАШ собственный лимит в 250 строк, а внутри "
               "нулевого блока строки идут по алфавиту — терялся весь "
               "словарь сайта после первых букв. Лимит снят; если доля всё "
               "ещё мала, теперь это действительно предел источника"
