...
# Сюда попадает то, что нельзя ни у кого спросить. Пусто — и это правильное
# состояние: каждый блокер, который МОЖНО вывести, выводится.
#
# Здесь стояли две рукописные строки про GA4, и они протухли ровно так же,
# как до них строка про доступ к Search Console: счётчик подключили — а
# доска продолжала бы объявлять его отсутствующим. Разница между «я записал»
# и «я спросил» в том, что первое стареет молча.
BLOCKERS_MEASURE = {}


def _ga4_blocker(key, root):
    """Строка про счётчик — или None, если счётчик стоит.

    Флаг читается ИЗ ИСХОДНИКА сайта (`render.ANALYTICS`), тем же разбором,
    которым читается число гейтов: доска собирается в CI, где исходников
    нет, поэтому снимок снимается здесь и печатается с датой.
    """
    if root is None:
        return ("счётчик не проверен: каталога сайта нет на машине, где "
                "снимался снимок. Это не «счётчика нет», это «не смотрели»")
    on = _const(os.path.join(root, "render.py"), "ANALYTICS")
    if on is True:
        return None
    if on is None:
        return ("счётчик не проверен: в render.py сайта не нашлось флага "
                "ANALYTICS — разбор молча вернул пустоту, и это провал "
                "разбора, а не ответ про счётчик")
    return ("GA4 не подключён: счётчика на страницах нет, и гейт сайта "
            "краснеет на любом счётчике, пока /privacy/ отрицает аналитику. "
            "Порядок обязателен и необратим по смыслу: сначала политика, "
            "потом счётчик, потом ресурс GA4")
...
def snapshot():
    ...
    for key, dec in DECLARED.items():
        ...
        if root is None:
            ...
            rec["missing"] = True
            if launched:
                rec["blockers"].append(_ga4_blocker(key, None))
        ...
        if launched:
            _ga = _ga4_blocker(key, root)
            if _ga:
                rec["blockers"].append(_ga)
